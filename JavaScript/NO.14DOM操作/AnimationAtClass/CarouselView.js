...
(function () {
    ...
    * */
    ...
    //    盛放所有指示视图的 数组
        ...
        this.clickImageCallback = new Function();
    ...
    * diretion 0向右 1向左
    * */
    CarouselView.prototype.update = function (diretion) {
        diretion = diretion||0;
        var self = this;
        if (diretion){
            new animte.Animation(700).moveOutToLeft(this.curImage,function () {
                //移除上一次显示的图片元素
                self.backgroundElement.removeChild(self.curImage);
            });

            // 把新的图片元素  添加到背景元素上
            this.backgroundElement.appendChild(this.imageElements[this.imageIndex]);

            new animte.Animation(700).moveInFromRight(this.imageElements[this.imageIndex],function () {
                // 更新this.curImage的值
                self.curImage = self.imageElements[self.imageIndex];
            });
        }else {
            new animte.Animation(700).moveOutToRight(this.curImage,function () {
                //移除上一次显示的图片元素
                self.backgroundElement.removeChild(self.curImage);
            });
            ...
            new animte.Animation(700).moveInFromLeft(this.imageElements[this.imageIndex],function () {
                // 更新this.curImage的值
                self.curImage = self.imageElements[self.imageIndex];
            });
        }
